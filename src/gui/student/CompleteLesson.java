...
for (CourseProgress cp : s.getProgress()) {
   ...
   private int countCompletedLessons(String courseId) {
    ...
    if (s.getProgress() == null) return 0;
    ...
    private void initComponents() {
        ...
        jButton5.setText("Display Attemps");
...
LessonProgress foundLessonProgress = null;

for (CourseProgress cp : s.getProgress()) {
    if (cp.getCourseId().equals(courseId)) {
        for (LessonProgress lp : cp.getLessonProgress()) {
            if (lp.getLessonId().equals(lessonId)) {
                foundLessonProgress = lp;
                break;
            }
        }
    }
...
if (foundLessonProgress == null) {
    JOptionPane.showMessageDialog(this, "No progress found for this lesson.", "Error",
            JOptionPane.ERROR_MESSAGE);
    return;
}

if (foundLessonProgress.getAttempts() == null || foundLessonProgress.getAttempts().isEmpty()) {
    JOptionPane.showMessageDialog(this, "No attempts for this lesson yet.", "Attempts",
            JOptionPane.INFORMATION_MESSAGE);
    return;
}
...
StringBuilder details = new StringBuilder();
details.append("=== Quiz Attempts ===\n\n");

int attemptNumber = 1;
for (QuizAttempt attempt : foundLessonProgress.getAttempts()) {
    details.append("Attempt ").append(attemptNumber++).append(": ");
    details.append(attempt.getQuizScore()).append("%\n");
}


JOptionPane.showMessageDialog(this,
        details.toString(),
        "Quiz Attempts for Lesson " + lessonId,
        JOptionPane.INFORMATION_MESSAGE);
